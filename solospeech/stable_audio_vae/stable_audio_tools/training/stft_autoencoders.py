...
from ..models.stftae import AudioAutoencoder
...
class AutoencoderTrainingWrapper(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.warmed_up and self.encoder_freeze_on_warmup:
            with torch.no_grad():
                latents, stds, encoder_info = self.autoencoder.encode(encoder_input, return_info=True)
        else:
            latents, stds, encoder_info = self.autoencoder.encode(encoder_input, return_info=True)
        ...
        if self.teacher_model is not None:
            with torch.no_grad():
                teacher_latents, teacher_stds = self.teacher_model.encode(encoder_input, return_info=False)
                loss_info['teacher_latents'] = teacher_latents
                loss_info['teacher_stds'] = teacher_stds
        ...
        decoded = self.autoencoder.decode(latents, stds)
        ...
        if self.teacher_model is not None:
            with torch.no_grad():
                teacher_decoded = self.teacher_model.decode(teacher_latents, teacher_stds)
                own_latents_teacher_decoded = self.teacher_model.decode(latents, stds) #Distilled model's latents decoded by teacher
                teacher_latents_own_decoded = self.autoencoder.decode(teacher_latents, teacher_stds) #Teacher's latents decoded by distilled model
        ...
        try:
            ...
            with torch.no_grad():
                if module.use_ema:
                    ...
                    latents, stds = module.autoencoder_ema.ema_model.encode(encoder_input)

                    fakes = module.autoencoder_ema.ema_model.decode(latents, stds)
                else:
                    latents, stds = module.autoencoder.encode(encoder_input)

                    fakes = module.autoencoder.decode(latents, stds)
